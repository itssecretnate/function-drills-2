...
function contains(arr, name, callback) {
  ...
  if(!containsName) arr.forEach(value => (value === name) ? containsName = true:false)

  callback(containsName)
}
...
function uniq(arr, callback) {
  let filteredNames = arr;

  for(let i = 0; i < arr.length; i++) {
    for(let j = i + 1; j < arr.length; j++) {
      if(arr[i] === arr[j])  {
        //console.log(`Duplicate name: ${arr[i]} at index: ${j}`)
        arr.splice(j,1);
      }
    }
  }
...
uniq(names, callback = uniqArr => console.log(`The new names array with all the duplicate items removed is ${uniqArr}`))
...
function getUserById(users, id, callback) {
  users.forEach(element => (element.id === id) ? callback(element):null)
}
...
const add17 = addingFactory(17);
console.log(add17(52))