...
const postTaxPrices = prices.map((element) => +(element = element * (1 + (taxRate / 100))).toFixed(2));
...
// CODE HERE
const totalPopulation = populations.reduce((total, curVal) => total += curVal)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((element) => (element.CP > 200) ? element:null)
...
const orderTotalsWithTax = []
orders.forEach((element) => {orderTotalsWithTax.push (+(element.price + (element.price + (1 + element.tax))))})
...
// CODE HERE
let bobsTotal = 0;
const bobs = purchases.filter( element => (element.owner === "Bob") ? element:null )
bobs.forEach(element => bobsTotal += element.price)
